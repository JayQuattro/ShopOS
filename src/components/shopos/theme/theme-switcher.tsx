...
export function ThemeSwitcher({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <label
      htmlFor={id}
      className={cn(
        "inline-flex items-center gap-2 rounded-md border border-border bg-card px-3 py-1.5 text-sm font-semibold text-card-foreground shadow-xs",
        compact && "px-2 py-1",
      )}
    >
      ...
      <select
        ...
        className="cursor-pointer rounded-sm bg-transparent py-0.5 pr-1 text-sm font-semibold"
        value={preference}
