...
const locationSchema = z.object({ locationId: z.string().trim().min(1) });
...
export async function DELETE(
  ...
  if (!locationId) {
    return Response.json({ error: "missing_locationId" }, { status: 400 });
  }
  ...
  try {
    ...
    await revokeLocationAccess({ db, context: tenantContext, membershipId, locationId });
    return Response.json({ ok: true }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    if (error instanceof MemberMutationFailed) {
      return Response.json({ error: error.reason }, { status: 404 });
