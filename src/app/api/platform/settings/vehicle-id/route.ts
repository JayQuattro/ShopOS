...
import {
  getPlatformVehicleIdConnector,
  upsertPlatformVehicleIdConnector,
  VehicleIdConnectorOperationFailed,
} from "@/modules/integrations/vehicle-id/vehicle-id-connector-service";
import { VEHICLE_ID_ADAPTER_DEFINITIONS } from "@/modules/integrations/vehicle-id/vehicle-id-connector-service";
import {
...
export async function GET(): Promise<Response> {
  try {
    ...
    const connector = await getPlatformVehicleIdConnector(db, context);
    return Response.json(
      { connector, adapters: VEHICLE_ID_ADAPTER_DEFINITIONS },
      { headers: { "Cache-Control": "no-store" } },
    );
  } catch (error) {
    return vehicleIdError(error);
  }
}
...
export async function PUT(request: Request): Promise<Response> {
  ...
  try {
    const context = await getPlatformRequestContext();
    const result = await upsertPlatformVehicleIdConnector({
      db,
      context,
...
function vehicleIdError(error: unknown): Response {
  ...
  if (error instanceof VehicleIdConnectorOperationFailed) {
