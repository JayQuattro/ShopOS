...
import {
  ...
  getPlatformEmailConnector,
  listEmailAdapterDefinitions,
  upsertPlatformEmailConnector,
} from "@/modules/platform/connectors";
...
export async function GET(): Promise<Response> {
  try {
    ...
    const [connector, adapters] = await Promise.all([
      getPlatformEmailConnector(db, context),
      listEmailAdapterDefinitions(db, context),
    ]);
    return Response.json({ connector, adapters }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
...
export async function PUT(request: Request): Promise<Response> {
  ...
  try {
    const context = await getPlatformRequestContext();
    const result = await upsertPlatformEmailConnector({
      db,
      context,
...
function platformError(error: unknown): Response {
  ...
  if (error instanceof ConnectorOperationFailed) {
